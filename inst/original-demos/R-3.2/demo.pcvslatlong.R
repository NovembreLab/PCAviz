...
# version 4.9 (on midway2, type "module load proj/4.9").
#
library(sp)
library(rworldmap)
source("../functions.R")
...
row.names(coldatamap) <- coldatamap$cntry
...
col   <- coldatamap[clabels,"color"]

# Remove all samples with missing geographic co-ordinates.
i       <- which(!is.na(xvals))
xvals   <- xvals[i]
yvals   <- yvals[i]
PCA     <- PCA[i,]
