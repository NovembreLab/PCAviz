...
# version 4.9 (on midway2, type "module load proj/4.9"). Note: since
# the map rendering can be very slow, I recommend using ThinLinc
# instead of X11 forwarding to display graphics.
#
library(sp)
library(rworldmap)
source("../functions.R")
...
# I think there must be a better way to replace these by calling a
# function. --Hossein
...
y <- colors[countrycolors[match(world_countries$NAME,names(countrycolors))]]
y[is.na(y)] <- "gainsboro"
